...
describe('unassignFromTask use case', () => {
  it('unassigns user on happy path', async () => {
    const repo = makeMockRepo()
    const task = makeFakeTask()

    vi.mocked(repo.findById).mockReturnValue(okAsync(task))
    vi.mocked(repo.unassignUser).mockReturnValue(okAsync(true))

    const useCase = createUnassignFromTask({ taskRepository: repo })
    const result = await useCase({ taskId: 'task-1', userId: 'user-2' })

    expect(result.isOk()).toBe(true)
    expect(repo.unassignUser).toHaveBeenCalledWith({ taskId: 'task-1', userId: 'user-2' })
  })
  ...
  it('returns TaskNotFound when task does not exist', async () => {
    ...
    const useCase = createUnassignFromTask({ taskRepository: repo })
    const result = await useCase({ taskId: 'nonexistent', userId: 'user-1' })

    expect(result.isErr()).toBe(true)
    if (result.isErr()) {
      expect(result.error.tag).toBe('TaskNotFound')
    }
    expect(repo.unassignUser).not.toHaveBeenCalled()
  })
  ...
  it('propagates repository errors', async () => {
    ...
    vi.mocked(repo.findById).mockReturnValue(okAsync(task))
    vi.mocked(repo.unassignUser).mockReturnValue(
